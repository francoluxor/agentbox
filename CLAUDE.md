...
## Important notes
 ...
 - For cloud work: the Daytona API key + org id and the Hetzner `HCLOUD_TOKEN` all live in `~/.agentbox/secrets.env` (managed by `agentbox daytona login` and `agentbox hetzner login` respectively). You may use the Daytona SDK directly via `node` + `@daytonaio/sdk` to inspect / clean up sandboxes when a test leaves an orphan, or `agentbox prune --provider daytona -y` for the supported path.
