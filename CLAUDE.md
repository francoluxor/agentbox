...
- Edits to the baked-in `/etc/claude-code/CLAUDE.md` hint, `/etc/profile.d/agentbox.sh` shim, or the `/etc/agentbox/` perms — all assembled by `RUN printf` blocks in `Dockerfile.box`. The runtime `box.env` is written per-create via `docker exec`, so its contents change without a rebuild; the shim that sources it does not.
