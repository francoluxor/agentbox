...
- [`docs/terminal-integration.md`](./docs/terminal-integration.md) — host-terminal attach placement (`--attach-in`/`attach.openIn` across tmux/cmux/iTerm2), terminal-title handling, and the **cmux sidebar box-status** integration (`attach.cmuxStatus` drives the workspace colour/description, not `set-status` pills) plus its gotchas (stored-but-hidden pills, socket session-auth, drive harness can't verify).
...
- [`docs/control-plane-roadmap.md`](./docs/control-plane-roadmap.md) — the **hosted control-plane** architecture + roadmap (serverless-on-Vercel and container-on-Hetzner topologies): the plane-as-custodian model, the verified pure-Node box-create + fire-and-poll bake findings, the nine concerns (relay forward, git-auth, creation, bake, HTTPS/DNS, agent creds, secret download, local tunnel, web UI), and the M1–M6 milestones. Pairs with [`docs/control-plane-backlog.md`](./docs/control-plane-backlog.md) (what shipped).
