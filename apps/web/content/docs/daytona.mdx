...
## Idle boxes stop themselves

A Daytona box auto-stops after **25 minutes idle** (`box.daytonaTimeoutMs`; `0` disables). While an agent is actually working, the host keeps the box alive, so only a genuinely idle box lapses. Stopping preserves the filesystem and frees the disk quota; `agentbox start` brings it back.
...
| Idle | Auto-stops after 25 min idle (`box.daytonaTimeoutMs`); held open while an agent is working |
