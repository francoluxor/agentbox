...
<Callout title="NOTE">
These are the AgentBox-provided skills, not the same thing as your own `~/.claude` skills — though those flow into every box too (see [Keeping boxes in sync](#keeping-boxes-in-sync)). For the agent session itself, see [Run an agent](/docs/run-an-agent).
</Callout>
...
Under the hood it calls `agentbox fork`. It's host-only in v1 (forking from inside a box isn't supported yet). The full flag reference — `--agent`, `--name`, `--plan`, and the rest — lives in [Run an agent → Fork a session](/docs/run-an-agent).
