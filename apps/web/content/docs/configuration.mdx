...
<Figure caption="Config resolution order: the highest layer that sets a key wins; unset keys fall through to the built-in default." />
...
| Key | Type | Default | Meaning |
| --- | --- | --- | --- |
| `attach.openIn` | enum `split`/`window`/`tab`/`same` | `split` | where `agentbox claude\|codex\|opencode` opens the attached session under tmux or iTerm2 |

`split` uses a tmux split-window / iTerm2 vertical split; `window` and `tab` open a new tmux window or tab. Outside tmux/iTerm2 every value behaves like `same`. See [access your box](/docs/access-your-box).
