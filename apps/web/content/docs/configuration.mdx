...
| `box.sizeDaytona` / `box.sizeHetzner` / `box.sizeDigitalocean` / `box.sizeVercel` / `box.sizeE2b` | string (advanced) | empty | per-provider override of `box.size` |
...
| `box.daytonaTimeoutMs` | int | `1500000` (25 min) | idle timeout before Daytona auto-stops the box; `0` disables. The host keepalive holds it open while the agent is working, so only a genuinely idle box lapses. Unlike vercel/e2b this is an *inactivity* window, not an absolute session TTL. Daytona-only |
...
| `box.imageDocker` / `box.imageDaytona` / `box.imageHetzner` / `box.imageDigitalocean` / `box.imageVercel` | string (advanced) | empty | per-provider override; the cloud ones are written by `agentbox prepare --provider <name>` |
