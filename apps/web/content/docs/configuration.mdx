...
| Key | Type | Default | Meaning |
| --- | --- | --- | --- |
| `box.size` | string | empty | generic cloud size, provider-interpreted: hetzner = server type (`cx33`), daytona = `cpu-memory-disk` GB (`4-8-20`), vercel = vCPU count (`1`/`2`/`4`/`8`), e2b = `cpu-memory` GB (`4-8`, baked at `prepare` time); docker ignores it |
| `box.sizeDaytona` / `box.sizeHetzner` / `box.sizeVercel` / `box.sizeE2b` | string (advanced) | empty | per-provider override of `box.size` |
