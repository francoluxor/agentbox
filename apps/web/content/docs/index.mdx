...
- **MacOS** or **Linux**
- An agent CLI — e.g. `claude`, `codex` or `opencode`. Agentbox will open a link for OAuth and acquire the token.
- (Optional) an account for Vercel, Hetzner or Daytona. Agentbox will open a link to retrieve the API key.
...
| `Ctrl+v`   | Paste an image or screenshot, even on clout boxes                      |
