...
If a box is fully stopped (or a cloud box idle-pauses and is later resumed),
`agentbox start` — and attaching — bring the agent back **resuming the same
conversation**, so it looks like the box never stopped. Claude reopens its exact
session; Codex resumes its most recent one.
