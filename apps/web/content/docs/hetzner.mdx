...
To reach a box from **another device** (a phone over Terminus/Blink, with your laptop off), open the firewall and hand the device a key: `agentbox inbound <box> open` then `agentbox connect <box> --add-key <device.pub>`. The baked sshd is key-only (`vscode`-only, no passwords), so `open` exposes port 22 to the internet but only your keys get in. See [remote access](/docs/access-your-box#use-a-box-with-your-laptop-offline).
