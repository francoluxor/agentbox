...
- **Direct** — the box holds a **copy of your git credentials** and pushes/pulls/signs entirely on its own — no host, no hub. See [Independent boxes](#independent-boxes--with-credentials) below.
...
### Independent boxes — `--with-credentials`

The relay and lease modes both keep your credentials off the box — but relay needs your PC on, and lease needs a hosted control plane. For a cloud box you intend to leave running unattended (Hetzner, or a pause/resume box on E2B/Vercel/Daytona), **`--with-credentials`** copies your own git credentials **into the box** so it can push, pull, and sign commits on its own, with your PC off and no hub:

```bash
agentbox create --provider hetzner --with-credentials
agentbox claude  --provider e2b     --with-credentials
```

At create time AgentBox detects what the repo's origin needs and, **after a confirmation prompt**, copies it into the box: a token (for HTTPS remotes, read from your git credential helper or `gh`), your SSH key (for SSH remotes), and your SSH signing key (if commit signing is on). Inside the box, `git push`/`fetch`/`pull` then run real git against the credentialed remote — the relay is never involved.

<Callout type="warn" title="This copies real credentials into the box">
Unlike every other mode, `--with-credentials` places your credentials **inside the box** — where its user has passwordless sudo (no boundary) — and they are captured in **any snapshot or checkpoint** of it. Only use it for a box you trust to run unattended. On a non-TTY (CI) the copy never happens silently: pass `--with-credentials-yes` or set `AGENTBOX_WITH_CREDENTIALS_YES=1` to opt in.
</Callout>

What still needs your PC: `agentbox cp`/`download`, `checkpoint`, and pull-request ops (`gh pr create`) remain host-relay operations — with the PC off they fail with a clear message rather than hanging. Only `git push`/`fetch`/`pull` are independent. `--with-credentials` is cloud-only (a docker box already runs on your host).
