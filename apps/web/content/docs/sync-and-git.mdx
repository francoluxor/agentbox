...
This is about the box's **outbound** git — it does **not** open the firewall or make the box reachable. To connect *to* the box from another device (a phone) with the laptop off, see [remote access](/docs/access-your-box#use-a-box-with-your-laptop-offline) (`agentbox inbound` + `agentbox connect`) — an independent axis you can combine with `--dangerously-with-credentials`.
