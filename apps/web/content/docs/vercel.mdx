...
On Vercel, `--size` is the **vCPU count** — one of `1`, `2`, `4`, or `8` (RAM is coupled at 2048 MB/vCPU; the default is 2). Set it per box with `--size 4`, or pin it with `box.sizeVercel` / the generic `box.size`. An unsupported value (e.g. `3`, or a foreign spec like a hetzner `cx33`) fails locally before the billable `Sandbox.create` call, naming the rejected value. Session length (`box.vercelTimeoutMs`) and egress lock (`box.vercelNetworkPolicy`) are separate Vercel-only keys — see [configuration](/docs/configuration) for the full list and precedence. Region is fixed at `iad1`.

```bash
agentbox create -y --provider vercel --size 4
agentbox config set box.sizeVercel 4 --project
...
- **`--size` on Vercel means vCPUs** — `1`/`2`/`4`/`8` only; other counts (or a size spec meant for another provider) fail fast before create.
