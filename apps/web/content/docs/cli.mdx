...
agentbox open 2 --in codex    # auto-open Codex's add-SSH-connection form (Hetzner boxes)
...
`open --in <app>` opens the box in a host app instead of Finder: `codex` writes the box's SSH alias and auto-opens Codex's add-SSH-connection deep link (needs a persistent SSH key, so Hetzner boxes only); `herdr`, `cmux`, and `iterm2` open a new workspace/window in that terminal app running `agentbox attach <box>` (the box is auto-started first); `vscode` is equivalent to `agentbox code`. `open --targets [--json]` reports which of these apps are installed on the host — the AgentBox Tray app uses it to decide which "Open In…" menu items to show. cmux rejects control connections from outside the app by default: enable `socketControlMode: automation` (or set a socket password) in cmux's settings for `--in cmux` to work.
