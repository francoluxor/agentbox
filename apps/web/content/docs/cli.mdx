...
```bash
# Create the GitHub App that leases push tokens + write the deploy env
agentbox control-plane setup

# Point boxes + the CLI at a deployed control plane (sets relay.controlPlaneUrl)
...
The optional [control plane](/docs/control-plane) is a hosted service (Next.js + Postgres) that holds the centralized concerns — git-token leasing via a GitHub App, permission state, the box registry/events — so cloud boxes keep pushing and opening PRs with your laptop off. `setup` runs the GitHub App creation flow in your browser and writes the deploy env; `set-url` points boxes at the deployment; `worker` is the long-running process that provisions boxes from the plane's queue.
