...
The `agent` group is a scripting surface for the agent's live state: `agent state`, `agent wait-for <state>`, and `agent get-plan-question`, all with `--json`. It also exposes the **host-action approvals** an orchestrator answers on a box's behalf: `agent approvals [box]` lists pending confirmations (git push, cp, gh PR writes, checkpoint) with their full command + argv, and `agent approve <id>` (or `--deny` / `--cancel`) answers one. Pair it with `agentbox drive` (snapshot the screen / send keystrokes) to drive one agent from another — see [Orchestration & queue boxes](/docs/background-and-parallel).
