...
**Provider sugar.** `agentbox <provider> <subcommand>` is rewritten to `--provider <provider>` before parsing. So `agentbox hetzner claude`, `agentbox vercel codex`, and `agentbox docker create` all work — they're identical to passing `--provider`.
...
Each agent command has three subcommands: `attach` reattaches to a running session (never re-syncs config), `start` (re)starts a session in an existing box (rsyncs host config unless `--no-sync-config`), and `login` runs that agent's auth flow.

```bash
# Name the box and drop into a shell once it's ready
agentbox create -n smoke --attach
...
<Figure caption="agentbox claude creates the box and drops you into a detachable Claude Code session." />
...
<Figure caption="agentbox dashboard — the box list plus the selected box's live session." />
...
| --- | --- |
| `list` / `ls` | `-j, --json`, `-g, --global`, `--watch`, `--live` |
| `status` | `-j, --json`, `--inspect` |
| `top` | `--once`, `-j, --json`, `--interval <s>`, `--live` |
| `logs` | `-n, --tail <n>`, `-f, --follow`, `--daemon` |
| `wait` | `--timeout <ms>`, `--units <names...>`, `-j, --json` |

The `agent` group is a scripting surface for the agent's live state: `agent state`, `agent wait-for <state>`, and `agent get-plan-question`, all with `--json`. Pair it with `agentbox drive` (snapshot the screen / send keystrokes) to drive one agent from another — see [Orchestration & queue boxes](/docs/background-and-parallel).
...
# Copy single files (direction inferred from the box: prefix)
...
`download` pulls `/workspace` back into your host workspace, honouring `.gitignore`. It has typed subcommands for agent state: `download env`, `download claude`, `download codex`, `download opencode`, `download config`. `cp` copies individual files in either direction, like `docker cp`. `git` runs git/gh operations against the box from the host through the relay, so push and fetch use your host credentials and the box never sees them.
...
`install` is the first-run setup wizard (system check, pick a provider, log in, prepare its base image, install the host skill). `doctor` diagnoses system and provider readiness. `prepare` builds base images or snapshots — omit `--provider` for status only.
...
`docker` is the default and is pure sugar (no login). Each cloud provider has a `login` subcommand (run by default); hetzner adds `firewall`, daytona adds `resync`.

See [Local Docker](/docs/local-docker), [Hetzner](/docs/hetzner), [Daytona](/docs/daytona), and [Vercel](/docs/vercel).
