...
The host resolver blocks `..` traversal, denies `/proc|/sys|/dev|/etc/passwd|/etc/shadow`, caps each entry at 50 MiB (raise with `AGENTBOX_CARRY_MAX_BYTES`), and flags symlinks that escape `$HOME` and the project root. On `create`, `claude`, `codex`, and `opencode` the host prompts once — listing each src→dest with size, mode, and symlink warnings — before copying.
...
<Callout type="warn" title="HEADS UP">
`carry` deliberately bypasses `.gitignore` and copies host files into the box. Treat the box as the untrusted side and only carry what the agent genuinely needs (credentials, env files). The per-entry cap is 50 MiB unless you raise `AGENTBOX_CARRY_MAX_BYTES`.
</Callout>
