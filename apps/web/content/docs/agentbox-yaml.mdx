...
All the usual service fields (`ready_when`, `restart`, `backoff`, `needs`, `expose`, `autostart`) still apply at the service level — only the container config (`name`/`ports`/`env`/`args`/`container_name`) nests under `image:`. The container is **reused by name** across box stop/start (its data lives in the per-box docker volume, which a checkpoint does not capture — see the database note in [services and tasks](/docs/services-and-tasks)).
