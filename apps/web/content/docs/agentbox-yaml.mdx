...
**2. In-box (files already in the workspace).** `agentbox-ctl render` is a declarative `sed` replacement — handy for rendering a gitignored `.env` from a committed `env.example` on every boot. The render is itself idempotent (the regex rules re-pin the same lines on every boot), so this task needs no `idempotent:` guard:
