...
<Figure
  src="/screenshots/herdr-permission-prompt.png"
  caption="Boxes across several projects running in Herdr — each shows up as a normal agent in Herdr's sidebar (left). The focused box's Claude has just run `gh pr create`, so AgentBox's host relay blocks on an approval band in the footer — you approve it on the host."
/>
...
A [Herdr plugin](https://herdr.dev/docs/plugins) adds a boxes overlay, two shortcuts, and a Ctrl+click handler. Install it either way:
...
<Figure
  src="/screenshots/herdr-overlay.png"
  caption="The boxes overlay (centre) lists every box grouped by project with its agent's live status; Herdr's sidebar agent panel (left, scope “all”) shows the same boxes as normal agents."
/>
