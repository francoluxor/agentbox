...
<Callout type="info" title="Host capability">These two endpoints run on the host that owns Docker/SSH and writes `~/.agentbox` — the local or full-host hub. On the hosted (serverless) plane they return `503`.</Callout>
...
<Callout type="info" title="What's not here yet">The structured mid-run prompt/link interaction stream is on the roadmap. Today the API covers boxes, projects, approvals, create jobs, and provider install/bake.</Callout>
