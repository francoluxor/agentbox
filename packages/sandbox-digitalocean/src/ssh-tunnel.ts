 ...
 *
 * This is the load-bearing piece that makes the DigitalOcean provider's preview-URL
 ...
 * Namespaced under `digitalocean/` so a DigitalOcean droplet id can't collide
 * with a Hetzner server id sharing the same numeric value. */
export function defaultBoxSshDir(boxId: string): string {
  return resolve(homedir(), '.agentbox', 'digitalocean', 'boxes', boxId, 'ssh');
}
