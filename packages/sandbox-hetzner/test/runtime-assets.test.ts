...
function makeFakeRepo(): string {
  ...
  const root = mkdtempSync(join(tmpdir(), 'agentbox-hetzner-test-'));
  mkdirSync(join(root, 'packages/sandbox-hetzner/scripts'), { recursive: true });
...
describe('resolveRuntimeAssets', () => {
  ...
  it('prefers cliRuntimeRoot when provided', () => {
    ...
    const cliRuntime = mkdtempSync(join(tmpdir(), 'agentbox-hetzner-cli-'));
    mkdirSync(join(cliRuntime, 'hetzner/scripts'), { recursive: true });
    writeFileSync(join(cliRuntime, 'hetzner/scripts/install-box.sh'), 'cli-stub');
    ...
    expect(installAsset?.localPath).toBe(join(cliRuntime, 'hetzner/scripts/install-box.sh'));
