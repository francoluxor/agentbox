...
import { HetznerApiError } from '../src/client.js';
import { isRetriable, withHetznerRetry } from '../src/retry.js';
...
describe('isRetriable', () => {
  ...
  it('retries `locked` and `conflict` codes (server tells us to wait)', () => {
    expect(isRetriable(new HetznerApiError(409, 'locked', 'busy'), false)).toBe(true);
    expect(isRetriable(new HetznerApiError(409, 'conflict', 'busy'), false)).toBe(true);
  });

  it('classifies 5xx as ambiguous (retried only when caller opts in)', () => {
    const err = new HetznerApiError(502, 'http_502', 'bad gateway');
    expect(isRetriable(err, false)).toBe(false);
    expect(isRetriable(err, true)).toBe(true);
  });

  it('never retries permanent 4xx (auth, validation, not_found)', () => {
    expect(isRetriable(new HetznerApiError(401, 'unauthorized', 'token bad'), true)).toBe(false);
    expect(isRetriable(new HetznerApiError(404, 'not_found', 'no such'), true)).toBe(false);
    expect(isRetriable(new HetznerApiError(422, 'invalid_input', 'bad cidr'), true)).toBe(false);
  });
...
describe('withHetznerRetry', () => {
  it('returns the value on first success', async () => {
    const out = await withHetznerRetry(
  ...
  it('retries on a 429 then succeeds', async () => {
    ...
    const out = await withHetznerRetry(
      ...
      async () => {
        calls += 1;
        if (calls === 1) throw new HetznerApiError(429, 'rate_limit_exceeded', 'Slow down');
        return 'ok';
      },
    );
    expect(out).toBe('ok');
  ...
  it('does not retry a 401 even when retryOnAmbiguous is true', async () => {
    ...
    await expect(
      withHetznerRetry(
        ...
        async () => {
          calls += 1;
          throw new HetznerApiError(401, 'unauthorized', 'nope');
        },
      ),
  ...
  it('throws the last error after exhausting attempts', async () => {
    ...
    await expect(
      withHetznerRetry(
        ...
        async () => {
          calls += 1;
          throw new HetznerApiError(502, 'http_502', 'still bad');
        },
      ),
