...
- **npm users can't get VMs.** CI publishes only the native-install box image (the workflow passes no build-arg), and a VM base must boot from a published image — so `--claude-install npm` falls back to `container`. Fix is small: publish a second `-npm` tagged variant from `.github/workflows/box-image.yml`.
